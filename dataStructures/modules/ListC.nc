...
generic module ListC(typedef t, int n) { provides interface List<t>; }
...
implementation {
  ...
  command t List.front() { return container[0]; }

  // Peek tail
  command t List.back() { return container[size]; }
  ...
  command uint16_t List.size() { return size; }

  command t List.get(uint16_t position) { return container[position]; }

  command uint16_t List.maxSize() { return MAX_SIZE; }
}
