...
// "generic" modules require you to put parentheses at the end of the name
generic module SimpleSendP() {
  // provides shows the interface we are implementing. See
  // lib/interface/SimpleSend.nc to see what funcitons we need to implement.
...
implementation {
  ...
  event void sendTimer.fired() { post sendBufferTask(); }
   ...
   *
   *@return
   *	error_t - Returns SUCCESS, EBUSY when the system is too busy using the
   *radio, or FAIL.
   */
  error_t send(uint16_t src, uint16_t dest, pack * message) {
    if (!busy) {
